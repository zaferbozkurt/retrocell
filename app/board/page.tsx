...
export default function RetroBoardPage() {
  ...
  const canSeeFn = useCallback(
    (item: RetroItem) =>
      sm || item.authorId === currentUserId || item.revealed,
    [sm, currentUserId],
  );
  ...
  const pastRetroIds = new Set(
    retros.filter((r) => r.teamId === team.id && r.id !== retro.id).map((r) => r.id),
  );
  ...
  const hiddenCount = items.filter((i) => !canSeeFn(i)).length;
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 md:px-6">
          ...
          {sm ? (
            <button
              onClick={() => {
                ...
                store.revealAllItems(retro.id, anyHidden);
              }}
              ...
              title="Tüm kartların görünürlüğünü değiştir"
            >
              {items.some((i) => !i.revealed) ? (
                <>
      ...
      {!sm && hiddenCount > 0 ? (
        <div className="mt-4 flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
          ...
          {hiddenCount} kart Scrum Master tarafından henüz paylaşılmadı.
        </div>
      ) : null}
...
function NotesSidebar({
  ...
  const visibleNotes = notes.filter(
    (n) => isScrumMaster || n.authorId === currentUserId,
  );
...
function Board({
  ...
  currentUserId: string;
  isScrumMaster: boolean;
  canSeeFn: (item: RetroItem) => boolean;
}) {
...
// ─── Masked (other user's hidden) card ──────────────────────────────────────
...
function ItemCard({
  ...
  const canEdit = isOwn || isScrumMaster;
  const canDrag = canEdit;

  const onDragStart = (e: React.DragEvent) => {
    if (editing || actionMode || !canDrag) {
      e.preventDefault();
      return;
    }
    e.dataTransfer.effectAllowed = "move";
  ...
  return (
    <div
      draggable={!editing && !actionMode && canDrag}
      onDragStart={onDragStart}
      onDragEnd={() => setDragging(false)}
      ...
      <div className="px-3 py-2">
        ...
        <div className="mt-1.5 flex items-center justify-between text-[11px] text-slate-500">
            ...
            {item.column !== "action" ? (
              item.revealed ? (
                <Eye
                  ...
                  aria-label="Tüm takım görüyor"
                />
              ) : (
                <Lock
                  className="size-3 text-slate-400"
                  aria-label="Sadece sen ve scrum master görüyor"
                />
              )
            ) : null}
          </span>
          <div className="flex items-center gap-0.5">
            {!editing && !actionMode ? (
              <>
                {isScrumMaster && item.column !== "action" ? (
                  <button
                    onClick={() => store.revealItem(item.id, !item.revealed)}
